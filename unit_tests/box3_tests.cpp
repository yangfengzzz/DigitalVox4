...
TEST(Box3, Constructors) {
    {
        ...
        EXPECT_EQ(Point3D(), box.bound.lowerCorner);
        EXPECT_EQ(Point3D(1, 1, 1), box.bound.upperCorner);
    }
    
    {
        ...
        EXPECT_FALSE(box.isNormalFlipped);
        EXPECT_EQ(Point3D(-1, 2, 1), box.bound.lowerCorner);
        EXPECT_EQ(Point3D(5, 3, 4), box.bound.upperCorner);
    }
...
TEST(Box3, ClosestDistance) {
    ...
    EXPECT_DOUBLE_EQ(Point3D(-1, 3, 4).distanceTo(Point3D(-2, 4, 5)),
                     result0);
    
    double result1 = box.closestDistance(Point3D(1, 5, 0));
    EXPECT_DOUBLE_EQ(Point3D(1, 3, 1).distanceTo(Point3D(1, 5, 0)), result1);
    
    double result2 = box.closestDistance(Point3D(9, 5, 7));
    EXPECT_DOUBLE_EQ(Point3D(5, 3, 4).distanceTo(Point3D(9, 5, 7)), result2);
    
    double result3 = box.closestDistance(Point3D(-2, 2.4, 3));
    EXPECT_DOUBLE_EQ(Point3D(-1, 2.4, 3).distanceTo(Point3D(-2, 2.4, 3)),
                     result3);
    
    double result4 = box.closestDistance(Point3D(1, 2.6, 1.1));
    EXPECT_DOUBLE_EQ(Point3D(1, 2.6, 1).distanceTo(Point3D(1, 2.6, 1.1)),
                     result4);
    
    double result5 = box.closestDistance(Point3D(9, 2.2, -1));
    EXPECT_DOUBLE_EQ(Point3D(5, 2.2, 1).distanceTo(Point3D(9, 2.2, -1)),
                     result5);
    
    double result6 = box.closestDistance(Point3D(-2, 1, 1.1));
    EXPECT_DOUBLE_EQ(Point3D(-1, 2, 1.1).distanceTo(Point3D(-2, 1, 1.1)),
                     result6);
    
    double result7 = box.closestDistance(Point3D(1, 0, 3.5));
    EXPECT_DOUBLE_EQ(Point3D(1, 2, 3.5).distanceTo(Point3D(1, 0, 3.5)),
                     result7);
    
    double result8 = box.closestDistance(Point3D(9, -1, -3));
    EXPECT_DOUBLE_EQ(Point3D(5, 2, 1).distanceTo(Point3D(9, -1, -3)),
                     result8);
}
...
TEST(Box3, ClosestIntersection) {
    ...
    SurfaceRayIntersection3 result0 = box.closestIntersection(
                                                              Ray3D(Point3D(2, 1, 7), Vector3D(-1, -1, -1).normalized()));
    EXPECT_TRUE(result0.isIntersecting);
    EXPECT_DOUBLE_EQ(std::sqrt(3), result0.distance);
    EXPECT_VECTOR3_EQ(Point3D(1, 0, 6), result0.point);
    EXPECT_VECTOR3_EQ(Point3D(0, 1, 0), result0.normal);
    
    SurfaceRayIntersection3 result1 = box.closestIntersection(Ray3D(Point3D(2, -0.5, 8), Vector3D(-1, -1, 1).normalized()));
    EXPECT_TRUE(result1.isIntersecting);
    EXPECT_DOUBLE_EQ(std::sqrt(0.75), result1.distance);
    EXPECT_VECTOR3_EQ(Point3D(1.5, -1, 8.5), result1.point);
    EXPECT_VECTOR3_EQ(Point3D(0, -1, 0), result1.normal);
    
    SurfaceRayIntersection3 result2 = box.closestIntersection(Ray3D(Point3D(2, -2, 4), Vector3D(-1, -1, -1).normalized()));
...
TEST(Box3, BoundingBox) {
    ...
    EXPECT_EQ(Point3D(-1, 2, 3), boundingBox.lowerCorner);
    EXPECT_EQ(Point3D(5, 3, 7), boundingBox.upperCorner);
}

...
TEST(SurfaceToImplicit2, Intersects) {
    ...
    bool result0 =
    s2i.intersects(Ray2D(Point2D(1, 4), Vector2D(-1, -1).normalized()));
    EXPECT_TRUE(result0);
    
    bool result1 =
    s2i.intersects(Ray2D(Point2D(1, 2.5), Vector2D(-1, -1).normalized()));
    EXPECT_TRUE(result1);
    
    bool result2 =
    s2i.intersects(Ray2D(Point2D(1, 1), Vector2D(-1, -1).normalized()));

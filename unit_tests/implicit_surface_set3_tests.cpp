...
TEST(ImplicitSurfaceSet3, Intersects) {
    ...
    bool result0 = sset.intersects(
                                   Ray3D(Point3D(1, 4, 5), Vector3D(-1, -1, -1).normalized()));
    EXPECT_TRUE(result0);
    
    bool result1 = sset.intersects(
                                   Ray3D(Point3D(1, 2.5, 6), Vector3D(-1, -1, 1).normalized()));
    EXPECT_TRUE(result1);
    
    bool result2 = sset.intersects(
                                   Ray3D(Point3D(1, 1, 2), Vector3D(-1, -1, -1).normalized()));
...
TEST(ImplicitSurfaceSet3, ClosestIntersection) {
    ...
    SurfaceRayIntersection3 result0 = sset.closestIntersection(Ray3D(Point3D(1, 4, 5), Vector3D(-1, -1, -1).normalized()));
    EXPECT_TRUE(result0.isIntersecting);
    EXPECT_DOUBLE_EQ(std::sqrt(3), result0.distance);
    EXPECT_EQ(Point3D(0, 3, 4), result0.point);
    
    SurfaceRayIntersection3 result1 = sset.closestIntersection(Ray3D(Point3D(1, 2.5, 6), Vector3D(-1, -1, 1).normalized()));
    ...
    SurfaceRayIntersection3 result2 = sset.closestIntersection(Ray3D(Point3D(1, 1, 2), Vector3D(-1, -1, -1).normalized()));

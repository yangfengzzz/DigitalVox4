...
TEST(ImplicitSurfaceSet2, Intersects) {
    ...
    bool result0 =
    sset.intersects(Ray2D(Point2D(1, 4), Vector2D(-1, -1).normalized()));
    EXPECT_TRUE(result0);
    
    bool result1 =
    sset.intersects(Ray2D(Point2D(1, 2.5), Vector2D(-1, -1).normalized()));
    EXPECT_TRUE(result1);
    
    bool result2 =
    sset.intersects(Ray2D(Point2D(1, 1), Vector2D(-1, -1).normalized()));

...
TEST(Box2, Constructors) {
    {
        ...
        EXPECT_EQ(Point2D(), box.bound.lowerCorner);
        EXPECT_EQ(Point2D(1, 1), box.bound.upperCorner);
    }
    
    {
        Box2 box(Point2D(-1, 2), Point2D(5, 3));
        
        EXPECT_EQ(Point2D(-1, 2), box.bound.lowerCorner);
        EXPECT_EQ(Point2D(5, 3), box.bound.upperCorner);
    }
...
TEST(Box2, ClosestDistance) {
    ...
    EXPECT_DOUBLE_EQ(Point2D(-2, 4).distanceTo(Point2D(-1, 3)), result0);
    
    double result1 = box.closestDistance(Point2D(1, 5));
    EXPECT_DOUBLE_EQ(Point2D(1, 5).distanceTo(Point2D(1, 3)), result1);
    
    double result2 = box.closestDistance(Point2D(9, 5));
    EXPECT_DOUBLE_EQ(Point2D(9, 5).distanceTo(Point2D(5, 3)), result2);
    
    double result3 = box.closestDistance(Point2D(-2, 2.4));
    EXPECT_DOUBLE_EQ(Point2D(-2, 2.4).distanceTo(Point2D(-1, 2.4)), result3);
    
    double result4 = box.closestDistance(Point2D(1, 2.6));
    EXPECT_DOUBLE_EQ(Point2D(1, 2.6).distanceTo(Point2D(1, 3)), result4);
    
    double result5 = box.closestDistance(Point2D(9, 2.2));
    EXPECT_DOUBLE_EQ(Point2D(9, 2.2).distanceTo(Point2D(5, 2.2)), result5);
    
    double result6 = box.closestDistance(Point2D(-2, 1));
    EXPECT_DOUBLE_EQ(Point2D(-2, 1).distanceTo(Point2D(-1, 2)), result6);
    
    double result7 = box.closestDistance(Point2D(1, 0));
    EXPECT_DOUBLE_EQ(Point2D(1, 0).distanceTo(Point2D(1, 2)), result7);
    
    double result8 = box.closestDistance(Point2D(9, -1));
    EXPECT_DOUBLE_EQ(Point2D(9, -1).distanceTo(Point2D(5, 2)), result8);
}
...
TEST(Box2, Intersects) {
    ...
    bool result0 =
    box.intersects(Ray2D(Point2D(1, 4), Vector2D(-1, -1).normalized()));
    EXPECT_TRUE(result0);
    
    bool result1 =
    box.intersects(Ray2D(Point2D(1, 2.5), Vector2D(-1, -1).normalized()));
    EXPECT_TRUE(result1);
    
    bool result2 =
    box.intersects(Ray2D(Point2D(1, 1), Vector2D(-1, -1).normalized()));
...
TEST(Box2, ClosestIntersection) {
    ...
    EXPECT_VECTOR2_EQ(Point2D(1, 0), result0.point);
    EXPECT_VECTOR2_EQ(Point2D(0, 1), result0.normal);
    ...
    EXPECT_VECTOR2_EQ(Point2D(1.5, -1), result1.point);
    EXPECT_VECTOR2_EQ(Point2D(0, -1), result1.normal);
...
TEST(Box2, BoundingBox) {
    ...
    EXPECT_EQ(Point2D(-1, 2), boundingBox.lowerCorner);
    EXPECT_EQ(Point2D(5, 3), boundingBox.upperCorner);
}
...
TEST(Box2, Builder) {
    ...
    EXPECT_EQ(Point2D(-3.0, -2.0), box.bound.lowerCorner);
    EXPECT_EQ(Point2D(5.0, 4.0), box.bound.upperCorner);
}

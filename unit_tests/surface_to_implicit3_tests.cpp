...
TEST(SurfaceToImplicit3, ClosestIntersection) {
    ...
    SurfaceRayIntersection3 result0 = s2i.closestIntersection(Ray3D(Point3D(1, 4, 5), Vector3D(-1, -1, -1).normalized()));
    EXPECT_TRUE(result0.isIntersecting);
    EXPECT_DOUBLE_EQ(std::sqrt(3), result0.distance);
    EXPECT_EQ(Point3D(0, 3, 4), result0.point);
    
    SurfaceRayIntersection3 result1 = s2i.closestIntersection(Ray3D(Point3D(1, 2.5, 6), Vector3D(-1, -1, 1).normalized()));
    ...
    SurfaceRayIntersection3 result2 = s2i.closestIntersection(Ray3D(Point3D(1, 1, 2), Vector3D(-1, -1, -1).normalized()));

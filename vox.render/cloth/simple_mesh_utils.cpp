...
void Polygon::subdivideTriangle(std::vector<Polygon> &out) const {
    ...
    for (int i = 0; i < 3; i++) {
        out.push_back(Polygon(Point(mPoints[i].p),
                              Point(0.5f * (mPoints[(i + 1) % 3].p + mPoints[i].p)),
                              Point(0.5f * (mPoints[(i + 2) % 3].p + mPoints[i].p))));
    }
    out.push_back(Polygon(Point(0.5f * (mPoints[0].p + mPoints[1].p)),
                          Point(0.5f * (mPoints[1].p + mPoints[2].p)),
...
void PolygonMesh::generateRenderBuffers(RenderVertex **vertices, uint16_t **indices, int *vertexCount, int *indexCount) const {
...
void PolygonMesh::generateSmoothRenderBuffers(RenderVertex **vertices, uint16_t **indices, int *vertexCount, int *indexCount) const {
    std::vector<RenderVertex> verts;
    std::vector<uint16_t> inds;
    verts.reserve(mPolygons.size() * 3);
    verts.reserve(inds.size() * 3);

...
template<class T>
class IMATH_EXPORT_TEMPLATE_TYPE Box<Vec2<T>> {
public:
...
template<class T>
IMATH_HOSTDEVICE IMATH_CONSTEXPR14 inline Box<Vec2<T>>::Box(const Vec2<T> &point) IMATH_NOEXCEPT {
    min = point;
    max = point;
}
...
template<class T>
IMATH_HOSTDEVICE IMATH_CONSTEXPR14 inline Box<Vec3<T>>::Box(const Vec3<T> &point) IMATH_NOEXCEPT {
    min = point;
    max = point;
}

...
public:
    ...
    using ValueType = Eigen::Quaternion<T>;
    ...
    Quaternion(ValueType value_): value(value_) {}
    ...
    explicit Quaternion(const Eigen::Matrix<T, 3, 3>& m33);
    ...
    void set(const Eigen::Matrix<T, 3, 3>& matrix);
    ...
    Eigen::Matrix<T, 3, 3> matrix3() const;
    
    //! Converts to the 4x4 rotation matrix.
    Eigen::Matrix<T, 4, 4> matrix4() const;

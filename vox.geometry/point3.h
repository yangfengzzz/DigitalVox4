...
public:
    ...
    using ValueType = Eigen::Matrix<T, 3, 1>;
    ...
    constexpr Point() : value(0, 0, 0) {}
    
    //! Constructs point with given parameters \p x_, \p y_, and \p z_.
    constexpr Point(T x_, T y_, T z_) : value(x_, y_, z_) {}
    ...
    constexpr Point(const Point2<T>& v, T z_) : value(v.x(), v.y(), z_) {}

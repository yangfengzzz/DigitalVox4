...
template<class ISortElements, class ISortShared>
uint32_t reduceWarps(uint32_t threadIdx, uint32_t laneIdx, uint32_t warpIdx,
                     uint32_t warpLimit, uint32_t threadPos, uint32_t threadEnd,
                     ISortElements sortElements, uint32_t bit,
                     bool bOutput, uint32_t scanOut, ISortShared sortShared) {
...
#endif
        sortShared.setReduce(threadIdx, uint4(!!(keyDigit & 1) << laneIdx,
                                              !!(keyDigit & 2) << laneIdx,
                                              !!(keyDigit & 4) << laneIdx,
                                              !!(keyDigit & 8) << laneIdx));
    ...
    if (warpIdx < warpLimit)
#endif
        if (laneIdx < 16) sortShared.setReduce(threadIdx,
                                               sortShared.getReduce(threadIdx)
                                               | sortShared.getReduce(threadIdx + 16));
    simdgroup_barrier(mem_flags::mem_threadgroup);
#if USE_WARP_LIMIT
    if (warpIdx < warpLimit)
#endif
        if (laneIdx < 8) sortShared.setReduce(threadIdx,
                                              sortShared.getReduce(threadIdx)
                                              | sortShared.getReduce(threadIdx + 8));
    simdgroup_barrier(mem_flags::mem_threadgroup);
#if USE_WARP_LIMIT
    if (warpIdx < warpLimit)
#endif
        if (laneIdx < 4) sortShared.setReduce(threadIdx,
                                              sortShared.getReduce(threadIdx)
                                              | sortShared.getReduce(threadIdx + 4));
    simdgroup_barrier(mem_flags::mem_threadgroup);
#if USE_WARP_LIMIT
    if (warpIdx < warpLimit)
#endif
        if (laneIdx < 2) sortShared.setReduce(threadIdx,
                                              sortShared.getReduce(threadIdx)
                                              | sortShared.getReduce(threadIdx + 2));
    simdgroup_barrier(mem_flags::mem_threadgroup);
#if USE_WARP_LIMIT
    if (warpIdx < warpLimit)
#endif
        if (laneIdx < 1) sortShared.setReduce(threadIdx,
                                              sortShared.getReduce(threadIdx)
                                              | sortShared.getReduce(threadIdx + 1));
    simdgroup_barrier(mem_flags::mem_threadgroup);
    ...
    if (bOutput) {
        ...
        {
            uint32_t bits =
            ((keyDigit & 1) - 1 ^ ballot[0])
            & (!!(keyDigit & 2) - 1 ^ ballot[1])
            & (!!(keyDigit & 4) - 1 ^ ballot[2])
            & (!!(keyDigit & 8) - 1 ^ ballot[3]);
        ...
        simdgroup_barrier(mem_flags::mem_threadgroup); //to remove gScan read/write race condition
#if USE_WARP_LIMIT
        if (warpIdx < warpLimit)
#endif
            if (laneIdx < 16) {
                int scanIdx = scanOut + warpIdx + laneIdx * WarpsPerBlock;
                sortShared.setScan(scanIdx, sortShared.getScan(scanIdx) + popcount((mask1 ^ ballot[0])
                                                                                   & (mask2 ^ ballot[1])
                                                                                   & (mask4 ^ ballot[2])
                                                                                   & (mask8 ^ ballot[3])));
            }
...
void radixSort_BitCount(uint32_t threadIdx, uint32_t n, ISortElements sortElements,
    ...
    for (uint32_t bit = startBit; bit < endBit; bit += 4) {
        ...
        for (i = 0, threadPos = threadBeg; i < WarpsFactor; ++i, threadPos += 32) {
            warpCount += reduceWarps(threadIdx, laneIdx, warpIdx, WarpsPerBlock,
                                     threadPos, warpEnd, sortElements, bit, false, 0, sortShared);
        }
        if (WarpsRemain > 0) {
            warpCount += reduceWarps(threadIdx, laneIdx, warpIdx, WarpsRemain,
                                     threadPos, warpEnd, sortElements, bit, false, 0, sortShared);
        }
        ...
        
        {
            for (uint32_t offset = 1; offset < ScanCount; offset *= 2) {
                if (threadIdx < ScanCount) {
                    if (threadIdx >= offset)
                        sortShared.setScan(scanOut + threadIdx,
                                           sortShared.getScan(scanIn + threadIdx) + sortShared.getScan(scanIn + threadIdx - offset));
                    else

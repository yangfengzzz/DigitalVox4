...
void TeleportApp::_initializeCloth(EntityPtr entity, physx::PxVec3 offset) {
...
void TeleportApp::update(float delta_time) {
    ...
    physx::PxVec3 position(0.0f,0.0f, _time*-25.0f);
    physx::PxQuat rotation(_time * physx::PxPi * 0.5f, physx::PxVec3(0.0f, 1.0f, 0.0f));

    _clothActor.cloth->setTranslation(position);
    _clothActor.cloth->setRotation(rotation);
    
    ClothApplication::update(delta_time);
}

void TeleportApp::_teleport() {
    _time = 0.0f;
    physx::PxVec3 position(0.0f, 0.0f, _time*-10.0f);
    physx::PxQuat rotation(_time, physx::PxVec3(0.0f, 1.0f, 0.0f));

    //mClothActor[0]->mCloth->teleport(-mClothActor[0]->mCloth->getTranslation());
    _clothActor.cloth->teleportToLocation(position, rotation);
    //mClothActor[0]->mCloth->clearInertia();
    _clothActor.cloth->ignoreVelocityDiscontinuity();
}

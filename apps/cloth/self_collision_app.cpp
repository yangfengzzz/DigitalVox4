...
void SelfCollisionApp::_initializeCloth(EntityPtr entity, int index, physx::PxMat44 transform) {
    ...
    _clothActor[index].cloth->setSolverFrequency(120.0f);
    _clothActor[index].cloth->setSelfCollisionDistance(0.26);
    _clothActor[index].cloth->setSelfCollisionStiffness(0.95);
    ...
    _clothActor[index].cloth->setSelfCollisionIndices(selfCollisionIndicesRange);
...
void SelfCollisionApp::loadScene(uint32_t width, uint32_t height) {
    ...
    _initializeCloth(clothEntity, 0, posTrans);
}

...
void InterCollisionApp::_initializeCloth(EntityPtr entity, int index, physx::PxMat44 transform) {
...
void InterCollisionApp::loadScene(uint32_t width, uint32_t height) {
    ...
    _initializeCloth(clothEntity, 1, posTrans);
    posTrans.setPosition(physx::PxVec3(0.0f, 1.6f, -1.4f));
    _initializeCloth(clothEntity, 2, posTrans);
}

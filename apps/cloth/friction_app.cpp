...
void FrictionApp::_initializeCloth(EntityPtr entity, int index, physx::PxVec3 offset, float frictionCoef) {
    ...
    _clothActor[index].cloth->setFriction(frictionCoef);
...
void FrictionApp::loadScene(uint32_t width, uint32_t height) {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        _initializeCloth(clothEntity, i, physx::PxVec3(4.f + float(i) * spaceX, 4.f, -18.f), friction);
    }
}

...
public:
    ...
    ~DummyTask() {
        mTm = nullptr;
    } //Way to catch race conditions. Will usually crash on nullptr if the task gets deleted before the taskmanager is done with it.
...
public:
    ...
    void ParallelLoop(F const &function) {
        ...
        for (int j = 0; j < count; j++) {
            jobs[j].Initialize(this, [j, &finalJob, function](Job *) {
                function(j);
            });
